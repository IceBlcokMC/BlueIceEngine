...
interface HeaderMethod {
    funcName: string;
    isStatic: boolean;
}
type HeaderMethods = HeaderMethod[];

interface HeaderClassDefine {
    defineClass: string;
    defineVariable: string;
}

interface CppClassDefineConstructor {
    className: string; // UUIDAPI
    funcName: string; // make
    sourceCode: string; // &UUIDAPI::make
}
...
class Matcher {
     ...
     * @param str .h 文件内容
     */
    static getHeaderClass(str: string): HeaderClass {
     ...
     * @returns 方法名
     */
    static getHeaderMethods(str: string): HeaderMethods {
        const Function = /(static\s+)?METHODS\((\w+)\);/g;
        let result: HeaderMethods = [];
        let tmp;
     ...
     */
    static getHeaderClassDefine(str: string): HeaderClassDefine {
     ...
     * @returns ClassDefine 定义
     */
    static getCppClassDefine(str: string): string | null {
     ...
     */
    static getCppClassDefineConstructor(
        str: string
    ): CppClassDefineConstructor {
    ...
    static hasClassDefineInCpp(str: string): boolean {
        return this.getCppClassDefine(str) != null;
    }
}

type HeaderInfo = HeaderClass &
    HeaderClassDefine & {
        methods: HeaderMethods;
    };
type CppInfo = CppClassDefineConstructor & {};
...
class Generators {
     ...
     */
    static findFile(dir: string, file: string): string | null {
        const queue = [dir];
    ...
    static getHeaderInfo(code: string): HeaderInfo | null {
    ...
    static getCppInfo(code: string): CppInfo | null {
        return Matcher.getCppClassDefineConstructor(code);
    }

    static defineClass(headerInfo: HeaderInfo, cppInfo: CppInfo): string {
        const fix = (str: string) => str.replace(/API/g, "");
        const ClassDefine = (h: HeaderInfo) =>
            `ClassDefine<${h.defineClass}> ${h.className}::${h.defineVariable} = \n`;
        const defineClass = (h: HeaderInfo) =>
            `\tdefineClass<${h.defineClass}>("${fix(h.className)}")\n`;
        const constructor_ = (c: CppInfo) =>
            `\t\t.constructor(${c ? c.sourceCode : "nullptr"})\n`;
        const function_ = (h: HeaderMethod, className: string) =>
            `\t\t.${h.isStatic ? "function" : "instanceFunction"}("${
                h.funcName
            }", &${className}::${h.funcName})\n`;
        const extendsAnnotation = (cur: string, par: string) =>
            `\n\t\t// ${cur} extends ${par}\n`;
        const build = (h: HeaderInfo) => `\t\t.build();`;
        ...
        function genMethods(info: HeaderInfo, extends_ = false) {
        ...
        function genExtends(currentClass: HeaderInfo, parentClass: HeaderInfo) {
            def += extendsAnnotation(
            ...
            if (parentClass.extends) {
                const file = Generators.findFile(
                    baseDir,
                    `${parentClass.extends}.h`
                );
...
function processFile(headerFilePath: string) {
    ...
    if (
        headerInfo.defineClass != headerInfo.className &&
        headerInfo.defineClass != "void"
    ) {
        throw new Error(
            `static ClassDefine<${headerInfo.defineClass}> != ${headerInfo.className}`
        );
    }
    ...
    const cppFilePath = path.join(
        ...
        path.basename(headerFilePath).replace(".h", ".cc")
    );
